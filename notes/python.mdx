...
| :--- | :--- | :--- | :--- | :--- |
| `int` | Numeric | ❌ No | `5` | Whole numbers |
| `float` | Numeric | ❌ No | `5.5` | Decimal numbers |
| `str` | Text | ❌ No | `"Hi"` | Text characters |
| `list` | Sequence | ✅ Yes | `[1, 2]` | Ordered collection |
| `tuple` | Sequence | ❌ No | `(1, 2)` | Fixed ordered collection |
| `dict` | Mapping | ✅ Yes | `{"a": 1}` | Key-value pairs |
| `bool` | Boolean | ❌ No | `True` | Logic (True/False) |
| `set` | Set | ✅ Yes | `{1, 2}` | Unique items only |
...
| Source Data | Syntax Example | Items Added to Set | Result Type |
| :--- | :--- | :--- | :--- |
| **Dictionary** | `set.update(mydict)` | The **Keys** only | `str`, `int`, etc. |
| **Dict Values** | `set.update(mydict.values())` | The **Values** only | `str`, `int`, etc. |
| **Dict Items** | `set.update(mydict.items())` | **(Key, Value) Pairs** | `tuple` |
...
<mark>The child's \_\_init\_\_() function overrides the inheritance of the parent's \_\_init\_\_() function.</mark>
