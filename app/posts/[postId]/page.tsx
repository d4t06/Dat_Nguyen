...
// import { getPostByName, getPostsMeta } from "@/libs/postLocal";
   ...
   return (
      <div className="w-2/4 mx-auto">
         ...
         <div className="flex flex-row gap-5 mt-8">
            {!!meta.tags &&
               meta.tags.map((tag,i) => {
                  return <Link className="text-l text-white hover:underline border px-3 py-1 rounded" href={`/tag/${tag}`} key={i}>#{tag}</Link>;
               })}
         </div>
         <Link href={"/"} className="text-white hover:underline text-xl block mt-5 mb-10 font-medium">
            Go back home
         </Link>
      </div>
   );
